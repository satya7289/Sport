        ...
        if request.method=="POST":
            ...
            if not checkout.checkin_status:
                ...
                for i,checkout_item in enumerate(item_list):
                    ...
                    prev_available=item.available
                    prev_description=item.description
                    prev_sport_type=item.sport_type

                    if quality!=prev_quality:
                        try:
                            check_item=Item.objects.filter(sport_type=prev_sport_type,name=prev_name,brand=prev_brand,quality=quality).first()
                            if quality=="Lost":
                            ...
                            new_item=Item(name=prev_name,brand=prev_brand,quantity=quantity,available=available,quality=quality,sport_type=prev_sport_type,created_at=D.datetime.now().date())
                            new_item.save()
                    else:
                ...
                try:
                    ...
                    checkin=Checkin(checkout_item=checkout,remarks=remarks,created_at=D.datetime.now().date())
                    checkin.save()
