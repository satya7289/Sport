...
def Search(request, *arg, **kwargs):
    if request.method=="GET":
        ...
        r_quantity=request.GET.get("quantity")
        ...
        try:
            ...
            if sport and r_item and r_brand and r_quality:
                item =Item.objects.filter(sport_type=sport,name=r_item,brand=r_brand,quality=r_quality,available__gte=0).first()
                types=item.available
                message="success"
                status=1
            elif sport and r_item and r_brand:
                items =Item.objects.filter(sport_type=sport,name=r_item,brand=r_brand,available__gte=0)
                types =[]
                for item in items:
